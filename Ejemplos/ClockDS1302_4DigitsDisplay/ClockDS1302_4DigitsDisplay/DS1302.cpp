...
Time::Time (const __FlashStringHelper* date, const __FlashStringHelper* time) {
...
void Time::SetTime(const char* time) {
  this->hr  = conv2d(time);
  this->min = conv2d(time + 3);
  this->sec = conv2d(time + 6);
}

void Time::SetDate(const char* date) {
  // sample input: date = "Dec 26 2009", time = "12:34:56"
  // or date="26-12-2009"
  this->yr = conv2d(date + 9);
  // Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec
  this->date = conv2d(date + 4);
  switch (date[0]) {
    case 'J': this->mon = date[1] == 'a' ? 1 : this->mon = date[2] == 'n' ? 6 : 7; break;
    case 'F': this->mon = 2; break;
    case 'A': this->mon = date[2] == 'r' ? 4 : 8; break;
    case 'M': this->mon = date[2] == 'r' ? 3 : 5; break;
    case 'S': this->mon = 9; break;
    case 'O': this->mon = 10; break;
    case 'N': this->mon = 11; break;
    case 'D': this->mon = 12; break;
    default:
      this->yr = conv2d(date + 8);
      this->date = conv2d(date);
      this->mon = conv2d(date + 3);
  }

  this->day = dayOfWeek(this->date, this->mon, this->yr);
}

Time::Day Time::dayOfWeek(int d, int m, int y) { 
    static int t[] = { 0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4 }; 
    y -= m < 3; 
    static const int day = (y + y/4 - y/100 + y/400 + t[m-1] + d) % 7;
